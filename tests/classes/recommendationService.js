class RecommendationService {
  ...
  _filterByCategory(category) {
    return this.factsDB.filter(fact => fact.category === category.toLowerCase());
  }

  _validateFavoritesInput(userId, factId) {
  ...
  generateFact(category) {
    ...
    const filteredFacts = this._filterByCategory(category);
  ...
  getFactsByCategory(category, limit) {
    ...
    const facts = this._filterByCategory(category);
    return facts.slice(0, limit);
  }

  saveToFavorites(userId, factId) {
    this._validateFavoritesInput(userId, factId);
